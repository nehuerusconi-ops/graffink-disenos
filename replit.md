...
- Storefront contact info: the only public contact channel is the email `graffink.desing@gmail.com`. It is rendered in the Footer (with a `Mail` icon) and in `CustomDesign.tsx` (CTA button + helper text). WhatsApp was intentionally removed across the storefront — do not re-add unless the operator explicitly asks.
