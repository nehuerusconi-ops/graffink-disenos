...
- Plancha agrupada: cart-level toggle that bundles all selected designs into a single "plancha" priced at `planchaGroupingPrice`. Client sends `groupAsPlancha: boolean` to `/api/payments/mercadopago/preference` and `/api/payments/paypal/create-order`; the server fetches the plancha price server-side (never trusts client) and overrides the order total + persists `orders.is_plancha_grouped`. Email + PDF render a single "Plancha agrupada (N diseños)" line with each included design listed below.
